    ...
    public class DxQueryable<T> : ISqlQueryable<T>
    {
        ...
        public string ToPagingSql(int pageIndex, int pageSize = 20)
        {
            ...
            sb.AppendFormat(@";WITH T AS(
                    ...
                    SELECT * FROM T,T2 WHERE T.RowNum BETWEEN {4} AND {5}", _propertyMapping.PropertiesString, OrderByColumns, _propertyMapping.ClassName, WhereClause, (pageIndex - 1) * pageSize +1, pageIndex * pageSize);
            return sb.ToString();
        }
